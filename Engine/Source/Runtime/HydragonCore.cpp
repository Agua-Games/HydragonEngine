// HydragonCore.cpp
//
// Implementation of the headless render core (generic engine initialization for Google Filament).
...
void RenderCore::attachSurface(void* nativeWindow, uint32_t width, uint32_t height) {
    mWidth  = width;
    mHeight = height;
...
void RenderCore::shutdown() {
    ...
    if (mCamera)   mEngine->destroyCameraComponent(mCameraEntity);
    if (mCameraEntity) utils::EntityManager::get().destroy(mCameraEntity);
