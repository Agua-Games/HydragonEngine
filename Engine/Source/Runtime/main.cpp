...
int main()
{
    ...
    if (core.init()) {
        std::printf("Filament integration: ENABLED and RenderCore initialized successfully (backend=Vulkan)\n");
        core.shutdown();
    } else {
        std::printf("Filament integration: ENABLED but RenderCore initialization FAILED\n");
    }
#else
