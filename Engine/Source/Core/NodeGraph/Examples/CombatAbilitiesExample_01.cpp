...
auto frostBolt = Scene::current()
    .add<Ability>("frost_bolt")
        ...
        .connect<MagicAttackAbility>("frost_chain")
            .maxTargets(3)
            .chainRange(5.0f)
            .damageReduction(0.2f)  // Each subsequent target takes 20% less damage
            .connect<StateModifier>("frozen")
                .duration(2.0f)
...
auto whirlwind = Scene::current()
    .add<Ability>("whirlwind")
        ...
        .connect<DamageZone>("spin_damage")
            .radius(3.0f)
            .connect<StateModifier>("bleeding")
                .duration(4.0f)
                .tickRate(0.5f)
        .connect<StateModifier>("rage")
            .value(5.0f)
            .connect<StateModifier>("berserk")
                .threshold(50.0f)
                .damageBonus(0.3f)
...
auto shadowStrike = Scene::current()
    .add<Ability>("shadow_strike")
        ...
        .damage(75.0f)
        .range(10.0f)
        .connect<Character>("dash")
            .speed(50.0f)
            .connect<StateModifier>("invulnerable")
                .duration(0.5f)
        .connect<StealthMechanics>("shadow_form")
            .fadeTime(0.3f)
            .connect<ParticleEmitter>("shadow_trail")
                .rate(50)
                .lifetime(0.5f)
        .connect<ComboMechanics>("backstab")
            .multiplier(2.0f)
            .condition("behind_target")
            .connect<StateModifier>("exposed")
                .duration(3.0f)
...
auto holyShield = Scene::current()
    .add<Ability>("holy_shield")
        ...
        .connect<DamageAbsorber>("divine_shield")
            .capacity(100.0f)
            .connect<HealingSpell>("divine_response")
