...
public:
    ...
    template<typename T>
    T getLocalProperty(const std::string& name, const T& defaultValue = T()) const {
...
// Mirrors a pxr::UsdPrim for data authority. Does not have spatial transformation.
...
public:
    ...
    template<typename T>
    std::shared_ptr<T> connect(std::shared_ptr<T> target, const char* port = nullptr) {
