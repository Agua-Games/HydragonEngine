 ...
 * 
 * @file WaveletMeshNode.h
 * @brief WaveletMeshNode is a mesh node which uses wavelet compression.
 * 
 * ARCHITECTURAL NOTES:
 * - Extends MeshNode to support wavelet compression
 * - Note that we have a node even more effective than WaveletMeshNode: AdaptiveMeshNode. It supports wavelet compression plus other techniques.
 ...
 * - Integrates with existing MeshNode system
...
class WaveletMeshNode : public Node {
public:
    explicit WaveletMeshNode(const WaveletMeshInfo& info = WaveletMeshInfo())
        : Node(info) {}
