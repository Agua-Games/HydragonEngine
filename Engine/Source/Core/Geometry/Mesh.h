 ...
 * 
 * @file Mesh.h
 * @brief Mesh represents a mesh node in the engine's node graph.
 * 
 * ARCHITECTURAL NOTES:
...
public:
    ...
    void processNodeGraph() override {
        ...
        auto deformers = getInputValue<std::vector<std::shared_ptr<Deformer>>>("Deformers");
        processDeformerChain(deformers);
        
        // Apply material and finish processing
        auto material = getInputValue<std::shared_ptr<Material>>("Material");
...
private:
    void processDeformerChain(const std::vector<std::shared_ptr<Deformer>>& deformers);
    void updateOutputs();
};
