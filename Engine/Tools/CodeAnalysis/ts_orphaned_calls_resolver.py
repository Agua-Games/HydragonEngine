...
TypeScript orphaned call resolver implementation.
"""
...
function {func_name}() {{
    throw new Error('Function {func_name} not implemented');
}}
"""
                ...
                with open(call_file, 'w', encoding='utf-8') as f:
                    for line in content:
                        if func_name in line:
                            ...
                            if f"import {{ {func_name} }}" not in content:
                                f.write(f"import {{ {func_name} }} from './{target_file.stem}';\n")
                            modified = True
                        f.write(line)
    ...
    def _determine_implementation_location(self, call_locations: List[Path]) -> Path:
        ...
        return call_locations[0].parent / "orphaned_implementations.ts"
