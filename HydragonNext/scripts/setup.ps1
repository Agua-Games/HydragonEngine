<#
    ...
    Usage (from a Developer PowerShell):
        ...
        ./scripts/setup.ps1 -FilamentVersion v1.51.0
#>
[CmdletBinding()]
param(
    [string]$VcpkgRoot = $env:VCPKG_ROOT,
    [string]$FilamentVersion = "v1.51.0",
    [switch]$SkipFilament
)

$ErrorActionPreference = "Stop"
$root = Split-Path -Parent $PSScriptRoot   # HydragonNext/
...
# --- 2. Filament prebuilt -------------------------------------------------
if (-not $SkipFilament) {
    $filamentDir = Join-Path $root "ThirdParty\Filament"
...
Write-Host "`nSetup complete. Open HydragonNext.sln in Visual Studio 2022." -ForegroundColor Green
