...
#: Paths of the trigger colliders this module authored. Kept SEPARATE from `_authored`, which also
#: holds the wireframes - a consumer asking "is this a game volume's collider?" must not get a yes for
#: a wireframe. See `is_trigger_collider` for the consumer this exists for.
_trigger_colliders: set = set()
...
def forget(path: str) -> None:
    ...
    for key in derived:
        ...
        _trigger_colliders.discard(key)


def is_trigger_collider(path: str) -> bool:
    """True when `path` names a trigger collider generated by this module.

    Exists for the camera's anti-clipping probe. A game volume is a PHYSICS region with no geometry
    inside it, so honouring one as an obstacle rushes the camera into the player the moment the
    player enters the volume: the probe ray starts at the player, which is now INSIDE the cage, so
    the first hit is the cage's own wall at whatever distance that happens to be.

    It has to be a filter in the CALLBACK, not a collision channel, because the scene query API
    offers no channel. `get_physx_scene_query_interface().raycast_all(origin, dir, distance,
    reportFn, bothSides)` takes no filter argument, and the bindings expose no
    `PxQueryFilterCallback` at all (`_physx.pyi` mentions collision groups only for the debug-window
    setting), so a `UsdPhysics.CollisionGroup` has no way to reach a raycast in this build. The
    reporting callback is the only point of control we have - and it is O(1) against a set we
    already maintain while authoring.

    Tolerant of a child path, because a hit can name a prim UNDER the collider rather than the
    collider itself.
    """
    if not path or not _trigger_colliders:
        return False
    if path in _trigger_colliders:
        return True
    probe = path
    while "/" in probe:
        probe = probe.rsplit("/", 1)[0]
        if probe in _trigger_colliders:
            return True
    return False
...
def ensure_trigger(volume_prim, volume_type: str) -> Optional[Any]:
    ...
    _failures_reported.discard(path)
    _authored[path] = fingerprint
    # Register the collider so the camera's anti-clipping probe can recognise and ignore it. Added
    # here, on the one path that ends in a successfully authored collider, and removed by `forget`.
    _trigger_colliders.add(path)
    return prim
